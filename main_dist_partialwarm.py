...
def select_points_closest_to_centroids(coordinates, kmeans_labels, kmeans_centroids, num_points_per_cluster):
    """
    Selects a subset of points based on their original coordinates, mapped from the clusters found using the distance matrix.
    """
    selected_points = {l: [] for l in range(kmeans_centroids.shape[0])}

    # Map the cluster labels to their corresponding 2D coordinates
    for l in range(kmeans_centroids.shape[0]):
        cluster_points = np.where(kmeans_labels == l)[0]
        centroid_coordinates = np.mean(coordinates[cluster_points], axis=0)
        distances = np.linalg.norm(coordinates[cluster_points] - centroid_coordinates, axis=1)
...
def perform_gurobi_clustering(distance_matrix, kmeans_labels, num_clusters, selected_points, d_max_lower_bound, d_avg_lower_bound):
    """
    ...
    for l, points in selected_points.items():
        for i in points:
            x[i, l].start = 1  # Partial warm start with selected points
    ...
    M = np.max(distance_matrix)  # Maximum distance in the distance matrix
    ...
    for l in range(num_clusters):
        model.addConstr(quicksum(w[i, j, l] for i in range(num_points) for j in range(num_points) if i != j) == quicksum(distance_matrix[i, j] * y[i, j, l] for i in range(num_points) for j in range(num_points) if i != j), name=f"NewAvgDist_{l}")
    ...
    kmeans_obj = calculate_kmeans_objective(distance_matrix, kmeans_labels, num_clusters)
...
def find_best_k(distance_matrix, max_k):
    """
...
def main():
    ...
    distance_matrix = preprocessing.preprocess("djlorj.txt").values

    d_max_lower_bound = np.min(distance_matrix[np.nonzero(distance_matrix)])
    d_avg_lower_bound = np.mean(distance_matrix[np.nonzero(distance_matrix)])

    max_k = 20
    optimal_k = find_best_k(distance_matrix, max_k)

    num_clusters = optimal_k
    kmeans_labels, kmeans_centroids = perform_kmeans_clustering(distance_matrix, num_clusters)
    ...
    custom_clusters = perform_gurobi_clustering(distance_matrix, kmeans_labels, num_clusters, selected_points,
